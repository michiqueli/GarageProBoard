import { contrato } from '@gpb/contracts'
import { formatearCuit } from '@gpb/core'
import { useQuery } from '@tanstack/react-query'
import { type KeyboardEvent, useId, useRef, useState } from 'react'
import { usarSesion } from '../sesion/almacen.ts'
import { api } from '../sesion/cliente.ts'
import { usePuedeUsar } from '../sesion/permisos.ts'

export interface ProveedorElegido {
  id: string
  razonSocial: string
}

/**
 * Elegir un proveedor escribiendo su razón social o CUIT. Mismo recorrido de teclado que
 * `SelectorCliente`: se escribe, `Enter` elige el primero, `↓` baja a la lista.
 */
export function SelectorProveedor({
  etiqueta,
  valor,
  onChange,
  autoFocus = false,
}: {
  etiqueta: string
  valor: ProveedorElegido | null
  onChange: (p: ProveedorElegido | null) => void
  autoFocus?: boolean
}) {
  const id = useId()
  const [buscar, setBuscar] = useState('')
  const lista = useRef<HTMLUListElement>(null)
  const campo = useRef<HTMLInputElement>(null)
  const tenantId = usarSesion((e) => e.datos?.tenant.id)
  const puedeVer = usePuedeUsar(contrato.proveedores.listar)
  const texto = buscar.trim()

  const consulta = useQuery({
    queryKey: ['proveedores', tenantId, texto, 'activos', 'selector'],
    queryFn: () =>
      api.proveedores.listar({ pagina: 1, porPagina: 8, buscar: texto, estado: 'activos' }),
    enabled: puedeVer && !valor && texto.length >= 2,
  })

  if (valor) {
    return (
      <div className="grid gap-1">
        <span className="text-etiqueta font-medium text-texto-suave">{etiqueta}</span>
        <div className="flex h-campo items-center gap-2 rounded-base border border-borde bg-superficie-2 px-2.5 text-dato">
          <span className="truncate">{valor.razonSocial}</span>
          <button
            type="button"
            onClick={() => {
              onChange(null)
              setBuscar('')
              requestAnimationFrame(() => campo.current?.focus())
            }}
            className="ml-auto text-etiqueta text-marca hover:underline"
          >
            Cambiar
          </button>
        </div>
      </div>
    )
  }

  const botones = () => [...(lista.current?.querySelectorAll('button') ?? [])]

  function moverDesdeCampo(e: KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Enter') {
      e.preventDefault()
      const primero = consulta.data?.datos[0]
      if (primero) onChange({ id: primero.id, razonSocial: primero.razonSocial })
      return
    }
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      botones()[0]?.focus()
    }
  }

  function moverEnLista(e: KeyboardEvent<HTMLUListElement>) {
    if (e.key !== 'ArrowDown' && e.key !== 'ArrowUp') return
    e.preventDefault()
    const todos = botones()
    const actual = todos.indexOf(document.activeElement as HTMLButtonElement)
    if (e.key === 'ArrowUp' && actual <= 0) {
      campo.current?.focus()
      return
    }
    todos[actual + (e.key === 'ArrowDown' ? 1 : -1)]?.focus()
  }

  const resultados = consulta.data?.datos ?? []

  return (
    <div className="relative grid gap-1">
      <label htmlFor={id} className="text-etiqueta font-medium text-texto-suave">
        {etiqueta}
      </label>
      <input
        id={id}
        ref={campo}
        // biome-ignore lint/a11y/noAutofocus: se abre a pedido y el foco tiene que caer acá
        autoFocus={autoFocus}
        value={buscar}
        onChange={(e) => setBuscar(e.target.value)}
        onKeyDown={moverDesdeCampo}
        placeholder="Razón social o CUIT"
        autoComplete="off"
        className="h-campo rounded-base border border-borde bg-superficie-2 px-2.5 text-dato text-texto outline-none placeholder:text-texto-tenue focus-visible:border-marca"
      />
      {texto.length >= 2 && consulta.data && (
        <ul
          ref={lista}
          aria-label={`Proveedores que coinciden con ${texto}`}
          onKeyDown={moverEnLista}
          className="grid divide-y divide-borde-suave overflow-hidden rounded-base border border-borde bg-superficie"
        >
          {resultados.length === 0 && (
            <li className="px-2.5 py-2 text-etiqueta text-texto-suave">
              Ningún proveedor coincide con «{texto}». Si es nuevo, dalo de alta en Repuestos →
              Proveedores.
            </li>
          )}
          {resultados.map((p) => (
            <li key={p.id}>
              <button
                type="button"
                onClick={() => onChange({ id: p.id, razonSocial: p.razonSocial })}
                className="flex w-full items-baseline gap-2 px-2.5 py-1.5 text-left text-dato hover:bg-superficie-2 focus-visible:bg-superficie-2 focus-visible:outline-none"
              >
                {p.razonSocial}
                <span className="ml-auto font-mono text-etiqueta text-texto-tenue">
                  {p.tipoDocumento === 96 ? p.numeroDocumento : formatearCuit(p.numeroDocumento)}
                </span>
              </button>
            </li>
          ))}
        </ul>
      )}
    </div>
  )
}
